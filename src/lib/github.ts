...
type GithubActivityInsert = Database["public"]["Tables"]["github_activity"]["Insert"];
type GithubDailyMetricsInsert = Database["public"]["Tables"]["github_daily_metrics"]["Insert"];
...
function aggregateDaily(commits: PushCommit[]): DailyMetrics[] {
  ...
  for (const commit of commits) {
    const dateKey = commit.committedAt.slice(0, 10); // YYYY-MM-DD
